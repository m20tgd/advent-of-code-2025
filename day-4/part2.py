...
with open("input.txt", 'r') as file:
    total_accessible_paper_count = 0
    ...
    while True:
        ...
        new_grid = [['.' for _ in range(number_of_columns)]
                    for _ in range(number_of_rows)]
        for y in range(number_of_rows):
            for x in range(number_of_columns):
                if rows[y][x] == '@':
                    if count_paper_in_surrounding_coords(x, y, rows) < 4:
                        accessible_paper_count += 1
                    else:
                        new_grid[y][x] = '@'
        rows = new_grid
        total_accessible_paper_count += accessible_paper_count
        if accessible_paper_count == 0:
            break


print(total_accessible_paper_count)
